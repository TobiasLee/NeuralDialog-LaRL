        ...
        try:
            for epoch_id in range(self.rl_config.nepoch):
                ...
                while True:
                    ...
                    n += 1
                    if n % 50 == 0:
                        print("Reinforcement Learning {}/{} eposide".format(n,
                    ...
                    task_report, success, match = self.agent.run(batch, self.evaluator,
                    ...
                    if self.rl_config.record_freq > 0 and n % self.rl_config.record_freq == 0:
                        ...
                        v_success, v_match, v_bleu, v_f1 = self.generate_func(self.sys_model, self.val_data,
                                                                              self.sv_config, self.evaluator, None,
                                                                              verbose=False)
                        ...
                        test_loss = self.validate_func(self.sys_model, self.test_data, self.sv_config, use_py=True)
                        t_success, t_match, t_bleu, t_f1 = self.generate_func(self.sys_model, self.test_data,
                                                                              self.sv_config, self.evaluator, None,
                                                                              verbose=False)
                        self.ppl_test_file.write('{}\t{}\t{}\t{}\n'.format(n, np.exp(test_loss), t_bleu, t_f1))
                        self.ppl_test_file.flush()
...
def generate(model, data, config, evaluator, num_batch, dest_f=None):
    ...
    while True:
        ...
        if config.dec_use_attn:
            ...
            pred_attns = np.array(pred_attns, dtype=float).squeeze(2).swapaxes(0,
                                                                               1)  # (batch_size, max_dec_len, max_ctx_len)
        else:
            pred_attns = None
        # get context
